...
parser.add_argument('-ds', '--dataset_train', type=str, default="", help='Training set')
...
with open(Parameters().get_cat_to_num_info_file(args.dataset_train, args.peptide_type), mode='w') as encoding_file:

    for arg in vars(args):
        encoding_file.write(f"#{arg}={getattr(args, arg)}\n")
        print(f"{arg}={getattr(args, arg)}")

    data_loader = DataLoader(transformer=DataTransformer(), mutation_types=args.mutation_types,
                             response_types=['CD8', 'CD4/CD8', 'negative'], immunogenic=args.immunogenic,
                             min_nr_immuno=0, cat_encoders=None,  max_netmhc_rank=args.max_rank_netmhc)

    patients_train = \
        get_valid_patients(dataset=args.dataset_train, peptide_type=args.peptide_type) \
            if args.dataset_train and len(args.dataset_train) > 0 else get_valid_patients(peptide_type=args.peptide_type)
