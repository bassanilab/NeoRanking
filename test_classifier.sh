source configure.sh
ENCODING=$1
CLF_BIN_WC=$2  # one of: SVM SVM-lin RF CART ADA LR NNN XGBoost CatBoost TabNet DNN XGBoost
PEPTIDE_TYPE=$3
FEATURE_TYPE=$4
NORMALIZER="${5:-q}"
NR_NEGATIVES="${6:-100000}"
WORK_DIR="${7:-/home/localadmin/Priorization/Classifiers/}"
ALPHA="${8:-0.005}"
FILE_TAG="${9:-n}"
...
#cmd="PYTHONPATH=/home/localadmin/Priorization/python python3 Classifier/TestClassifiers.py -d $WORK_DIR -c $CLF_BIN_WC -s $SCORER -tr $TRAIN_SET -te $TEST_SET -i $INPUT_FILE_TAG -f $FEATURES -n $NORMALIZER -v 2 -a $ALPHA  -mt SNV -im CD8 CD4/CD8 $CAT_TO_NUM -pt $PEPTIDE_TYPE -eg $EXCL_GENES  -cv -nn $NR_NEGATIVES &"
cmd="PYTHONPATH=/home/localadmin/Priorization/python python3 Classifier/TestClassifiers.py -d $WORK_DIR -c $CLF_BIN_WC -s $SCORER -tr $TRAIN_SET -te $TEST_SET -i $INPUT_FILE_TAG -f $FEATURES -n $NORMALIZER -v 2 -a $ALPHA -mt SNV -im CD8 CD4/CD8 $CAT_TO_NUM -pt $PEPTIDE_TYPE -eg $EXCL_GENES -nn $NR_NEGATIVES &"
